from app.services.live import play_name_pattern
...
def test_rushing_td_with_missed_xp_by_someone_else():
    ...
    r = parse_play(t, play_name_pattern("Derrick Henry"), "BAL")
    assert r["summary"] == "46-yd rush, TD"
    assert r["delta"] == {"rush_att": 1, "rush_yd": 46, "rush_td": 1}
    assert play_points(r["delta"], None, 1.0) == 10.6
    assert parse_play(t, play_name_pattern("Tyler Loop"), "BAL")["delta"] == {"xpmiss": 1}
...
def test_receiving_td_both_sides():
    ...
    r = parse_play(t, play_name_pattern("Bijan Robinson"), "ATL")
    assert r["summary"] == "50-yd catch, TD" and r["delta"] == {"rec": 1, "rec_yd": 50, "rec_td": 1}
    q = parse_play(t, play_name_pattern("Michael Penix"), "ATL")
    assert q["summary"] == "50-yd TD pass to B.Robinson" and q["delta"] == {"pass_yd": 50, "pass_td": 1}
    assert parse_play(t, play_name_pattern("Younghoe Koo"), "ATL")["delta"] == {"xpm": 1}
...
def test_two_point_and_field_goal():
    ...
    assert parse_play(t, play_name_pattern("Jaylen Hill"), "MIA")["delta"] == {"rec_2pt": 1}
    assert parse_play(t, play_name_pattern("De'Von Achane"), "MIA")["summary"] == "11-yd catch, TD"
    fg = parse_play("J.Myers 37 yard field goal is GOOD, Center-C.Stoll, Holder-M.Dickson.", play_name_pattern("Jason Myers"), "SEA")
    assert fg["summary"] == "37-yd FG" and play_points(fg["delta"], None, 1.0) == 3


def test_no_impact_plays_return_none():
    assert parse_play("J.Allen pass incomplete short right to D.Kincaid.", play_name_pattern("Dalton Kincaid"), "BUF") is None
    assert parse_play("C.McLaughlin kicks 59 yards from TB 35 to ATL 6. J.Agnew to ATL 35 for 29 yards.", play_name_pattern("Chase McLaughlin"), "TB") is None


def test_fumble_lost_and_interception():
    f = parse_play("D.Henry right guard to BLT 40 for 3 yards. D.Henry FUMBLES, RECOVERED by BUF-T.Bernard at BLT 41.", play_name_pattern("Derrick Henry"), "BAL")
    assert f["delta"] == {"rush_att": 1, "rush_yd": 3, "fum_lost": 1}
    assert play_points(f["delta"], default_scoring(1.0), 1.0) == -1.7
    i = parse_play("J.Allen pass deep left intended for K.Coleman INTERCEPTED by M.Humphrey at BLT 20.", play_name_pattern("Josh Allen"), "BUF")
    assert i["delta"] == {"pass_int": 1}
    assert parse_play("J.Allen pass deep left intended for K.Coleman INTERCEPTED by M.Humphrey at BLT 20.", play_name_pattern("Keon Coleman"), "BUF") is None


def test_leading_parentheticals_are_stripped():
    r = parse_play("(Shotgun, No Huddle) D.Henry left end for 5 yards.", play_name_pattern("Derrick Henry"), "BAL")
    ...
    assert parse_play(t, play_name_pattern("James Cook"), "BUF")["delta"] == {"rec": 1, "rec_yd": 8, "rec_td": 1}
    assert parse_play(t, play_name_pattern("Josh Allen"), "BUF")["delta"] == {"pass_yd": 8, "pass_td": 1, "rush_2pt": 1}
    # A tackler parenthetical still gets removed mid-sentence, not treated as a leading tag.
    f = parse_play("(No Huddle) S.Barkley right guard to PHI 40 for 3 yards (M.Humphrey).", play_name_pattern("Saquon Barkley"), "PHI")
...
def test_parses_lengthened_initial_from_real_play_text():
    key = play_name_pattern("Bijan Robinson")
    r = parse_play("Bi.Robinson left tackle to ATL 34 for 4 yards (D.Wyatt).", key, "ATL")
    assert r["summary"] == "4-yd rush" and r["delta"] == {"rush_att": 1, "rush_yd": 4}
    c = parse_play("(Shotgun) M.Penix pass short left to Bi.Robinson to GB 49 for 17 yards (Z.Franklin; B.Cisse).", key, "ATL")
    assert c["summary"] == "17-yd catch" and c["delta"] == {"rec": 1, "rec_yd": 17}
    # A teammate's kickoff return must not be credited to him.
    assert parse_play("T.Smack kicks 59 yards from GB 35 to ATL 6. Br.Robinson to ATL 30 for 24 yards (B.Melton).", key, "ATL") is None
